...
namespace EntityFrameworkMock.NSubstitute.Tests
{
    public class DbContextMock<TDbContext> where TDbContext : DbContext
    {
        ...
        {
            DbContext = Substitute.ForPartsOf<TDbContext>(args);
        ...
        {
            _dbSetCache.Clear();
            DbContext.SaveChanges().Returns(a => SaveChanges());
            DbContext.SaveChangesAsync().Returns(a => SaveChanges());
            DbContext.SaveChangesAsync(Arg.Any<CancellationToken>()).Returns(a => SaveChanges());
        }
